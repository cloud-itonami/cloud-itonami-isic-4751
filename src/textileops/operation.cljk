(ns textileops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (TextileRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the TextileRetailGovernor (:govern)
  ...
  human store operator/textile-quality coordinator. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [textileops.advisor :as advisor]
            [textileops.governor :as governor]
            [textileops.phase :as phase]
...
(defn build
  ...
  `textileops.store/Store`).
  opts:
    :advisor      -- a `textileops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TextileRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; TextileRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
