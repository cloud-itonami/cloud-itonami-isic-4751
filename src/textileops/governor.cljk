(ns textileops.governor
  "TextileRetailGovernor -- the independent compliance layer that earns
  the TextileRetailAdvisor the right to commit. The advisor has no notion
  ...
  (sales/inventory/cut-yardage transaction logging, floor-staff
  scheduling, textile supply-order coordination, quality-concern
  flagging). It NEVER performs or authorizes:
    ...
    - quality-dispute-resolution authority (accepting/denying liability
      on the store's behalf, instructing a vendor's account be closed)
  ...
  Four HARD checks, ALL permanent, un-overridable by any human approval:
    ...
    2. Vendor unverified          -- for `:coordinate-supply-order` ONLY,
                                     ...
                                     (fabric mill/wholesaler) record. A
    ...
    4. Scope exclusion            -- ANY proposal (regardless of op)
                                     ...
                                     contract) is a HARD, PERMANENT block
                                     ...
                                     surfacing a defective-bolt/mislabeling/
                                     dye-lot-mismatch concern for a human
  ...
  Two ESCALATE (SOFT) gates, either forces human sign-off:
    ...
    - The op is `:flag-quality-concern` -- ALWAYS escalates to a human,
      ...
      otherwise is. `textileops.phase` independently agrees:
    ...
    - A `:coordinate-supply-order` whose drafted `:value` names an
      ...
      textile procurement proposal always needs a human sign-off, even
...
(def supply-cost-threshold
  "Example single-store textile procurement threshold (USD-equivalent
...
(def scope-excluded-terms
  ...
  registration/contract) or otherwise actuating on a quality concern
  ...
  ACTION (e.g. 'issue the refund', 'void the sale'), never a bare noun
  like 'refund', 'dispute', 'quality concern' or 'chargeback' -- a bare
  ...
  talk about defective bolts/mislabeling/dye-lot-mismatch quality
  concerns, and whose own printed `:op` keyword literally contains the
  substring 'quality-concern') and self-block the happy path. See
  `textileops.governor-test/default-mock-advisor-proposals-never-self-trip-scope-exclusion`
...
(defn- scope-exclusion-violations
  ...
  chargeback, vendor registration/contract revocation), regardless of
  ...
  (let [op (:op proposal)
    ...
    (cond
      ...
      [{:rule :scope-excluded
        :detail "返金/交換の実行・販売の無効化・仕入先チャージバック・登録取消など品質紛争確定行為(quality-dispute-resolution finalization)に触れる提案は永久に禁止"}])))
...
(defn check
  "Censors a TextileRetailAdvisor proposal against the governor rules.
