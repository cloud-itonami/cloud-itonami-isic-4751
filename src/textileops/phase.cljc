(ns textileops.phase
  "Phase 0->3 staged rollout for the ISIC-4751 'Retail sale of textiles in
  specialized stores' (fabric/notions retail) operations-coordination
  actor.
  ...
  rollout milestone still to come. Flagging a quality concern always
  needs a human to actually look at it. `textileops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently -- two
  layers, not one, agree on this."
  (:require [textileops.governor :as governor]))
...
(defn verdict->disposition
  "Map a TextileRetailGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
