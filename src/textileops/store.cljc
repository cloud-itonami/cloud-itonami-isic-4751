(ns textileops.store
  "SSoT for the ISIC-4751 'Retail sale of textiles in specialized stores'
  (fabric/notions storefront -- bolts of woven/knit fabric, thread,
  trims, patterns and sewing notions, distinct from ISIC 4771-family
  apparel retail which sells finished garments, not yardage) operations-
  ...
  This actor coordinates the back-office operations of a fabric/notions
  storefront: sales/inventory/cut-yardage transaction logging, floor-staff
  scheduling, textile supply-order coordination with registered vendors
  (fabric mills/wholesalers), and quality-concern flagging (a defective
  bolt, mislabeled fiber content, or a dye-lot mismatch). It never sets or
  authorizes a shelf/unit price, and never directly finalizes a
  quality-dispute resolution (issuing a refund/replacement, voiding a
  sale, charging back a vendor, revoking a vendor's registration) -- see
  `textileops.governor`'s `scope-exclusion-violations`, a HARD, permanent,
  ...
  commit or escalate -- `textileops.governor`'s `store-unverified-
...
(defn demo-data
  ...
  []
  {:stores
   {"store-1" {:store-id "store-1" :name "Riverside Fabric & Notions"
               :registered? true :verified? true}
    "store-2" {:store-id "store-2" :name "Sunset Boulevard Textile Supply"
               :registered? true :verified? true}
    "store-3" {:store-id "store-3" :name "Downtown Pop-Up Fabric Kiosk (in intake)"
               :registered? true :verified? false}}
   :vendors
   {"vendor-1" {:vendor-id "vendor-1" :name "Northgate Textile Mills Supply"
                :registered? true :verified? true}
    "vendor-2" {:vendor-id "vendor-2" :name "Unverified Import Fabric Broker Co."
                :registered? true :verified? false}}})
