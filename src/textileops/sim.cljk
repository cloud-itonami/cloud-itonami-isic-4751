...
(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "textile-quality-coordinator-1"}} {:thread-id tid :resume? true}))

(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :fabric-store-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :fabric-store-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 42 :yards-cut 118.5 :returns 3 :stock-count-delta -45}} coordinator-phase-1)]
      (println r)
      (println "-- human fabric store coordinator approves --")
    ...
    (println (exec-op actor "t2" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 30 :yards-cut 76.0 :returns 1 :stock-count-delta -31}} coordinator-phase-3))
    ...
    (println (exec-op actor "t3" {:op :schedule-staffing-operation :store-id "store-1"
                                  :patch {:shift "weekend-cutting-counter" :date "2026-07-20" :window "10:00-18:00"}} coordinator-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "quilting-cotton bolts restock" :quantity 40 :estimated-cost 360.0
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "wool-suiting bolts, seasonal collection" :quantity 60 :estimated-cost 2400.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human fabric store coordinator reviews & approves --")
    ...
    (let [r (exec-op actor "t6" {:op :flag-quality-concern :store-id "store-1"
                                 :patch {:concern "bolt #4471 shows a dye-lot mismatch against the swatch card, and the fiber-content label reads 100% cotton but the burn test suggests a poly blend" :confidence 0.9}} coordinator-phase-3)]
      (println r)
      (println "-- human fabric store coordinator reviews & approves --")
    ...
    (println (exec-op actor "t9" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "imported linen bolts" :quantity 25 :estimated-cost 300.0
    ...
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
      ...
      (println (exec-op actor-direct "t10" {:op :schedule-staffing-operation :store-id "store-1"
                                           :patch {:shift "weekday-cutting-counter" :date "2026-07-22"}} coordinator-phase-3)))
