(ns textileops.governor-test
  "Pure unit tests of `textileops.governor/check` against hand-built
  proposals -- the fast, focused complement to `governor-contract-test`'s
  full-graph integration coverage."
  (:require [clojure.test :refer [deftest is testing]]
            [textileops.advisor :as adv]
            [textileops.governor :as gov]
            [textileops.store :as store]))

(def store-1 {:store-id "store-1" :name "Riverside Fabric & Notions" :registered? true :verified? true})
(def store-3 {:store-id "store-3" :name "Downtown Pop-Up Fabric Kiosk" :registered? true :verified? false})
(def vendor-1 {:vendor-id "vendor-1" :name "Northgate Textile Mills Supply" :registered? true :verified? true})
(def vendor-2 {:vendor-id "vendor-2" :name "Unverified Import Fabric Broker Co." :registered? true :verified? false})

(defn- clean-proposal [op store-id]
  {:op op :store-id store-id :summary "s" :rationale "routine fabric store coordination"
  ...
  (testing "a proposal whose rationale touches directly issuing a refund is HARD-blocked regardless of op/confidence"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "issued the refund to the customer and confirmed the defective bolt claim"
                          :confidence 0.99)
  ...
  (testing "a proposal touching charging back the vendor is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          poisoned (assoc (clean-supply-order "store-1" "vendor-1" 100.0)
                          :summary "charged back the vendor for the defective bolt shipment")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))
...
(deftest legitimate-quality-concern-is-not-scope-excluded
  (testing "flagging an observed defective-bolt/mislabeling/dye-lot-mismatch concern (not a resolution finalization) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          concern (assoc (clean-proposal :flag-quality-concern "store-1")
                         :value {:concern "bolt #4471 dye-lot mismatch against swatch card, fiber-content label may be wrong"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (defective bolt/mislabeling/dye-lot mismatch) is exactly what this op exists to surface"))))
...
;; "refund" or "dispute"), which then accidentally matches inside the
    ...
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})]
      (doseq [op [:log-sales-record :schedule-staffing-operation :coordinate-supply-order
        ...
        (let [patch (if (= op :coordinate-supply-order)
                      {:item "quilting-cotton bolts restock" :estimated-cost 360.0 :vendor-id "vendor-1"}
                      {})
